...
<?php
$user = $row['username'];
$query = "SELECT * FROM files where username='$user' and privacy='Private' "; //You don't need a ; like you do in SQL
...
echo"<div class=\"header\"><h1>Your Private Files</h1></div>";
echo"<div class=\"content\">";
echo "<br></br><table id=\"mainTable\" class=\"table table-stripedd\">"; // start a table tag in the HTML
echo "<th>File Name</th><th>File Size</th><th>Download Link</th><th>Password</th>";
while($row = mysql_fetch_array($result)){   //Creates a loop to loop through results
	echo "<tr><td>" . $row['filename'] . "</td><td>" . $row['size']. "</td><td>" ."<a href='../files/".$row['short_id']."/".$row['short_id'].".zip"."'>Download</a>" . "</td><td>". $row['password'] . "</td><td>" ."</td></tr>";  //$row['index'] the index here is a field name
}
